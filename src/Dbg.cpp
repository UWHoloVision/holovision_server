...
void colorpoints_pipeline() {
  ...
  std::vector<std::string> colorframes {
    ...
    "262924111809", "262942429637", "262973736463",
  ...
  for (auto i=0; i< depthframes.size(); i++) {
    ...
    auto depthframe = depthframes[i];
    auto colorframe = colorframes[i];
    ...
    std::cout << "to rgb" << std::endl;
    pcl::copyPointCloud(*pointcloud, *colorcloud); // XYZ -> XYZRGB
